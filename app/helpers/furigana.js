...
export const rubyHtmlTransform = (rubyHtmlString, color = '') => {
  ...
  result = result.replace(/<rb>/g, '<div style="flex: 3; font-size: 20px;">');
  result = result.replace(/<rt>/g, '<div style="flex: 1; font-size: 10px;">');
  result = result.replace(/<\/ruby>/g, '</div></div>');
  result = result.replace(/<\/rb>/g, '</div>');
  result = result.replace(/<\/rt>/g, '</div>');
  result = `<div id="id" style="font-size: 20px !important; display: flex; flex-direction: row;">${result}</div>`;
  //   const xml = result;
